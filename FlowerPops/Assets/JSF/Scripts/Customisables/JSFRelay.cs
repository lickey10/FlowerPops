...
public static class JSFRelay {
	...
	public static void onComboEnd(){
		if(gm.currentCombo > 7){
            if(gm.audioScript.comboHighFx.Length > 0)
			gm.audioScript.comboHighFx[Random.Range(0,gm.audioScript.comboHighFx.Length-1)].play(); // play sound for hi combo
		} else if(gm.currentCombo > 5){
            if(gm.audioScript.comboMidFx.Length > 0)
			    gm.audioScript.comboMidFx[Random.Range(0,gm.audioScript.comboMidFx.Length)].play(); // play sound for mid combo
		} else if(gm.currentCombo > 3){
            if(gm.audioScript.comboLowFx.Length > 0)
			    gm.audioScript.comboLowFx[Random.Range(0,gm.audioScript.comboLowFx.Length-1)].play(); // play sound for low combo
		}
