...
public class JSFAudioPlayer : MonoBehaviour
{
    ...
    void loadBGM()
    {
        ...
        if (enableMusic && bgmPlayer.GetComponent<AudioSource>().clip != null)
        { // if music is enabled
    ...
    {
        if (bgmPlayer.GetComponent<AudioSource>().clip != null && bgmPlayer.GetComponent<AudioSource>().isPlaying)
        { // if music is playing
    ...
    {
        if (player == null)
        { // try and get it manually if player forgot to assign an AudioSource
